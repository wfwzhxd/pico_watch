...
{
#if INPUT_DEBUG
    printf("mouse_register begin\n");
#endif
    // only get once, it's good in rotate 0/180, we don't handle rotate 90/270
    hor = lv_disp_get_hor_res(NULL) - 1;
    ver = lv_disp_get_ver_res(NULL) - 1;
    
    input_key_init(mouse_keys, ARRAY_LEN(mouse_keys));
    lv_indev_drv_init(&lv_mouse_driver);
    lv_mouse_driver.type = LV_INDEV_TYPE_POINTER;
    lv_mouse_driver.read_cb = mouse_read;
    lv_mouse_indev = lv_indev_drv_register(&lv_mouse_driver);
    lv_obj_t * cursor_obj = lv_img_create(lv_scr_act());
    lv_img_set_src(cursor_obj, &mouse_cursor_icon);
    lv_indev_set_cursor(lv_mouse_indev, cursor_obj);
#if INPUT_DEBUG
    if (!lv_mouse_indev)
    {
        printf("lv_indev_drv_register failed\n");
    }
    printf("mouse_register done\n");
#endif
}

static lv_indev_state_t mouse_state = LV_INDEV_STATE_REL;
static lv_coord_t x,y;
#define MOUSE_STEP 2
...
int mouse_scan()
{
    ...
    {
        mouse_state = LV_INDEV_STATE_PR;
        keyCnt++;
    } else {
        mouse_state = LV_INDEV_STATE_REL;
    }
    ...
    if (IS_KEY_PRESSED(PIN_KEY_UP))
    {
        y -= MOUSE_STEP;
        keyCnt++;
    }
    if (IS_KEY_PRESSED(PIN_KEY_DOWN))
    {
        y += MOUSE_STEP;
        keyCnt++;
    }
...
void mouse_read(struct _lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    ...
    // printf("mouse_read x:%d y:%d, state: %d\n", data->point.x, data->point.y, data->state);
}

input_driver_t input_mouse = {
    .regist = mouse_register,
    .scan = mouse_scan};
